...
political_orientation = {"right": ['Le Figaro'],
                         "left": ['Libération'],
                         "center": ['Le Monde']}

names = {'Le Figaro': 'figaro', 'Libération': 'liberation', 'Le Monde': 'monde'}
...
def preprocess_corpus(corpus_path, output_path, ref_year):
    all_data = []
    all_sents = []
    ...
    for idx, row in data.items():
        if row['titre'] not in duplicates and row['annee']>=ref_year:
            ...
            po = ''
            po_name = ''
            if publisher in political_orientation['left']:
                po = 'left'
                po_name = names[publisher]
            elif publisher in political_orientation['right']:
                po = 'right'
                po_name = names[publisher]
            elif publisher in political_orientation['center']:
                po = 'center'
                po_name = names[publisher]
            ...
            if po_name:
                ...
                if output is not None:
                    ...
                    all_data.append((po_name, po, " <eos> ".join(text), " <eos> ".join(lemmatized_text)))
    ...
    df = pd.DataFrame(all_data, columns=['publisher', 'political_orientation', 'text', 'lemmatized_text'])
...
if __name__ == '__main__':
    ...
    ref_year = 2015
    folder = os.path.join('by_newspapers', 'since_2015', 'data')
    if not os.path.exists(folder):
        os.mkdir(folder)
    vocab_output = os.path.join(folder, 'by_paper_3_vocab.pickle')
    vocab_path = os.path.join(folder, 'by_paper_3_vocab.csv')
    output_path = os.path.join(folder, 'by_paper_3_preprocessed.tsv')

    preprocess_corpus(corpus_path, output_path, ref_year)
