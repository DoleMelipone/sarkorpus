...
ref_date = datetime.date(2012, 5, 12)
...
def preprocess_corpus(corpus_path, output_path):
    all_data = []
    all_sents = []
    ...
    for idx, row in data.items():
        if row['titre'] not in duplicates:
            ...
            if po_name and row['annee']>=2002:
                count_period[po_name] += 1
                date = datetime.date(row['annee'], row['mois'], row['jour'])
                if date <= ref_date:
                    p = 'political'
                else:
                    p = 'judicial'
                ...
                if output is not None:
                    ...
                    all_data.append((period, p, po_name, " <eos> ".join(text), " <eos> ".join(lemmatized_text)))
    ...
    df = pd.DataFrame(all_data, columns=['period', 'date', 'publisher', 'text', 'lemmatized_text'])
...
if __name__ == '__main__':
    ...
    folder = os.path.join('by_period', 'data')
    if not os.path.exists(folder):
        os.mkdir(folder)
    vocab_output = os.path.join(folder, 'by_date_vocab.pickle')
    vocab_path = os.path.join(folder, 'by_date_vocab.csv')
    output_path = os.path.join(folder, 'by_date_preprocessed.tsv')
