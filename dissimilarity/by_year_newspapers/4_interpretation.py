...
def get_clusters_sent(target, threshold_size_cluster, labels, sentences, id2sents, corpus_slices, docs_folder):
    ...
    sent_df = pd.DataFrame(list(zip(po, categs, topic, labels, sentences, lemmas)),
            columns=['po', 'newspaper', 'date', 'cluster_label', 'sentence', 'lemmatized_sent'])
    sent_df.to_csv(os.path.join(docs_folder, target + '_sentences.tsv'), encoding='utf-8', index=False, sep='\t')
    return sent_df


def output_distrib(data, word, keyword_clusters, image_folder, translator, corpus_slices):
...
def full_analysis(word, labels, sentences, id2sent, corpus_slices, image_folder, docs_folder, max_df=0.7, topn=15, threshold_size_cluster=5, lang='sl', translator=None):
    ...
    for k in keyword_clusters:
        ...
        print(keywords)
    output_distrib(clusters_sents_df, word, keyword_clusters, image_folder, translator, corpus_slices)
...
if __name__ == '__main__':
    ...
    for year in years:
        for comp in comps:
            ...
            interpretation_folder = os.path.join('by_year_newspapers', comp, str(year), 'interpretation')
            if not os.path.exists(interpretation_folder):
                os.makedirs(interpretation_folder)

            results_folder = os.path.join('by_year_newspapers', comp, str(year), 'results')
            ...
            target_words = get_target_words(os.path.join(results_folder, 'results_pretrained_selected_words.csv'), 'JSD K5 Avg', treshold=1)
