    ...
    for doc, lemmatized_doc, meta in ds:
        ...
        for sent, lemmatized_sent in zip(sents, lemmatized_sents):
            ...
            if len(lsent.split()) > 2:
                ...
                #sentence by sentence, comment this and uncomment the thing below if needed
                #indexed_tokens = tokenizer.convert_tokens_to_ids(tokenized_sent)
                # print("Batch counter: ", len(tokenized_text), batch_counter, tokenized_text)
                #batch.append((indexed_tokens, mapping))
                #batch_counter += 1
...
def get_token_embeddings(batches, model, batch_size, device):
    ...
    for batch in batches:
        ...
        with torch.no_grad():
            ...
            encoded_layers = model_output[-1][-6:-2]  #last four layers of the encoder
...
def get_slice_embeddings(embeddings_path, vocab, tokenizer, model, batch_size, max_length, device):
    vocab_vectors = {}
    count2sents = {}
    not_in_targets = set()
    ...
    for chunk in vocab.chunks:
        ...
        for batches in chunked_batches:
            ...
            encoder_token_embeddings, mappings, tokens = get_token_embeddings(batches, model, batch_size, device)
...
if __name__ == '__main__':
    ...
    comps = ['libe-fig', 'libe-monde', 'monde-fig']

    for year in years:
        for comp in comps:
            file = os.path.join('by_year_newspapers', comp, str(year), 'data', 'by_year_vocab.pickle')
            with open(file, 'rb') as handle:
                vocab = dill.load(handle)

            dir_path = os.path.join('by_year_newspapers', comp, str(year), 'embeddings')
            embeddings_path = os.path.join(dir_path, 'by_year_vocab.pickle')
            tokenizer = AutoTokenizer.from_pretrained("camembert-base", use_fast=False)
            model = AutoModelForMaskedLM.from_pretrained("camembert-base", output_hidden_states=True)
            model.to(device)
            if not os.path.exists(dir_path):
                os.mkdir(dir_path)
            get_slice_embeddings(embeddings_path, vocab, tokenizer, model, batch_size, max_length, device)
